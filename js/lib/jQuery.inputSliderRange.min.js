...
config.self.val(config.current)}}
function getTransformValue(toCount,percentage,min,max,startNumbStepping,step,stepRoundOnInput,inputTypeScroll,roundSize=null){if(typeof step==='object'){return transformValueWithProportionSteps(...arguments)}
let stepNew={};stepNew[min]=step;return transformValueWithProportionSteps(toCount,percentage,min,max,startNumbStepping,stepNew,stepRoundOnInput,inputTypeScroll,roundSize)}
function transformValueWithProportionSteps(toCount,percentage,min,max,startNumbStepping,step,stepRoundOnInput,inputTypeScroll,roundSize=null){step=getValidSteps(step,min,max);const outputPackage=(percentRaw,valueRaw,percent,value)=>({percentRaw,valueRaw,percent,value});toCount=parseFloat(toCount);let rawValue,percent;if(percentage){percent=validatePercent(toCount);rawValue=findRawValueByPercentInLineSegment(percent,min,max,step)}else{rawValue=parseFloat(validateValue(toCount,min,max))}
if(Number.isInteger(roundSize)){rawValue=parseFloat(rawValue.toFixed(roundSize))}if(inputTypeScroll===1||inputTypeScroll===0){stepRoundOnInput=!0}
let rawPercent=getPercentByValueInLineSegment(rawValue,step,min,max);if(stepRoundOnInput){let mostAppropriateValue=findMostAppropriateValueWithStepsInLineSegment(rawValue,step,min,max,startNumbStepping);if(Number.isInteger(roundSize)){mostAppropriateValue=parseFloat(mostAppropriateValue.toFixed(roundSize))}let mostAppropriatePercent=getPercentByValueInLineSegment(mostAppropriateValue,step,min,max);return outputPackage(rawPercent,rawValue,mostAppropriatePercent,mostAppropriateValue)}
...
function getConfigFromAttr($element){let attributesName=['min','max','step','theme'],dataAttributesName = ['roundSize'],objectFormAttr={};attributesName.forEach(attributeName=>{let valueAttribute=$element.attr(attributeName);if(typeof valueAttribute!=='undefined'){objectFormAttr[attributeName]=parseFloat(valueAttribute)}});dataAttributesName.forEach(dataAttributeName=>{let valueDataAttribute=$element.data(dataAttributeName);if(typeof valueDataAttribute!=='undefined'){objectFormAttr[dataAttributeName]=parseFloat(valueDataAttribute)}});return objectFormAttr}
